...
| 板块 | 页面 | 干什么 | 用不用 AI |
|---|---|---|---|
| 帮 | 项目切换 / 开店设置 | 统一管理多家店，三步新增项目并在顶部随时切换 | ✅ |
| 帮 | 开店清单 | 五阶段任务逐项打勾,带金额的合计就是开店账 | ✅ 可重新定制 |
| 帮 | 预算测算 | 客流客单价 / 时段预约双收入模型 + 保本刻度尺 + 现金流 | ✅ 预算体检 |
| 帮 | 选址分析 / 平台选择 | 联动当前项目业务和预算的六维评分，候选参数可同步预算 | ✅ |
| 帮 | 风险识别 | 联动项目品类、预算及选址报告，健康分 + 五维雷达 | ✅ 深度扫描 |
| 帮 | AI 参谋 | 双人设对话,系统提示词注入你的真实经营数字 | ✅ |
| 自定义 | 模型设置 | 换供应商、换模型、测连通性 | — |
| 自定义 | 数据存储 | 默认 SQLite 持久化，也可连接 MySQL | — |

每个店铺项目独立保存开店清单、预算参数、选址 / 平台候选和 AI 参谋对话。顶部项目选择器切换店铺后，「帮」板块会整体切换到该项目的数据与 AI 上下文；旧版线上 / 线下单店快照会自动迁移为项目列表。
...
- 业务快照包含统一的开店项目列表，以及每个项目独立的清单、预算、候选方案和 AI 对话
