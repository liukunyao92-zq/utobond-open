...
/** 把用户配置补全成可直接发请求的形态 */
export function resolvePreset(config = {}) {
  const preset = PRESETS[config.provider] || PRESETS.custom;
  return {
    provider: preset.id,
    kind: config.kind || preset.kind,
    baseURL: stripTrailingSlash(config.baseURL || preset.baseURL),
...
export function isConfigured(config) {
  ...
  return r.keyOptional ? true : !!r.apiKey;
}
